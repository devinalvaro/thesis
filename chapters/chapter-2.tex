...
Seperti \en{neural network} pada \en{shallow learning}, setiap hubungan neuron antar \en{layer} memiliki bobot (\en{weight}) yang nilainya merepresentasikan signifikansi hubungan antar neuron tersebut. Selain bobot, terdapat juga bias yang secara langsung mempengaruhi signifikasi setiap neuron (tidak seperti bobot yang mempengaruhi hubungan antara neuron). Secara garis besar, tujuan setiap neuron dan hubungan antar neuron adalah mengenali pola atau fitur dari data sehingga dapat memprediksi \en{output} dengan benar \citep{nielsen2015neural}.

Tujuan pelatihan \en{neural network} itu sendiri adalah mencari kombinasi bobot dan bias setiap neuron dan hubungannya sedemikian sehingga dari input dihasilkan \en{output} yang paling mendekati target. Ukuran dekatnya \en{output} dengan target adalah \en{loss} atau \en{cost function}, yaitu fungsi yang menghasilkan selisih antara \en{output} dan target. Semakin kecil \en{cost}, maka model semakin optimal.
...
Dapat diperhatikan bahwa untuk mendapatkan $\delta_{j}$ diperlukan $\delta_{k}$ untuk semua $k$, yaitu indeks neuron-neuron pada \en{layer} sesudahnya. Demikianlah mengapa teknik ini disebut \en{backpropagation} karena pencarian gradien yang dilakukan dengan mempropagasikan \en{error} dari \en{layer} belakang ke depan. Seperti yang telah disebutkan, gradien yang didapat kemudian digunakan untuk meng-\en{update} parameter bersangkutan.
...
$w_{ji}$ menghubungkan neuron berindeks $i$ dengan neuron berindeks $j$ pada \en{layer} sesudahnya bukan hanya pada satu \en{timestep} saja, melainkan di seluruh \en{timestep} (seperti yang telah disebutkan, bobot yang sama digunakan untuk seluruh \en{timestep}). Maka dari itu, terdapat hubungan dengan seluruh neuron (berindeks $l$) pada seluruh \en{timestep} (berindeks $t - \tau$). Hubungan ini memberikan lanjutan persamaan (\ref{eq:W}) berikut.
...
\section{\en{Graphics Processing Unit}} \label{GPU}

Penggunaan GPU pada pelatihan \en{deep learning} merupakan salah satu faktor yang memajukan perkembangan bidang ini secara signifikan \citep{dean2012large}. Hal ini dikarenakan GPU mampu meningkatkan kecepatan dengan menjalankan pelatihan secara paralel. Sayangnya, ukuran memori GPU lebih terbatas  dari memori CPU. Umumnya, memori GPU berukuran 8-16 GB sedangkan memori CPU dapat mencapai 1 TB.
...
Selain solusi-solusi tersebut, terdapat juga metode \en{memory swapping}, yaitu memindahkan sebagian isi memori dari memori GPU ke CPU saat pelatihan. Akan tetapi, \en{memory swapping} yang terlalu sering dapat menurunkan kecepatan pelatihan secara signifikan karena adanya \en{memory transfer overhead} \citep{dean2012large}.
...
Pada CUDA, CPU disebut \en{host} sehingga memori CPU disebut \en{host memory} dan GPU disebut \en{device} sehingga memori GPU disebut \en{device memory}. Pada umumnya, kode yang dijalankan di \en{host} tidak dapat langsung mengakses \en{device memory} dan begitu juga sebaliknya. Untuk itu, perlu dilakukan transfer memori dari \en{device} ke \en{host} atau sebaliknya. Penjelasan lebih lanjut mengenai transfer memori ini akan dibahas pada bagian selanjutnya.

Sejak CUDA 6 terdapat fitur \en{unified memory}, yaitu abstraksi memori yang dapat diakses secara langsung baik oleh \en{host} maupun \en{device}. Di balik layar, tetap dilakukan transfer memori antar \en{host} dan \en{device}, namun secara otomatis dilakukan oleh kernel CUDA, sehingga memudahkan \en{programmer} meskipun cenderung mengakibatkan kinerja yang lebih buruk.

\subsubsection{\en{Asynchronous Memory Copy}}

CUDA mendukung \en{asynchronous memory copy} dengan \code{cudaMemcpyAsync}, yaitu fungsi penyalinan isi memori antara \en{host} dan \en{device} yang tidak mem-\en{block} pemanggilnya.

Mekanisme \en{memory copy} ini adalah sebagai berikut. Misalnya dilakukan \en{memory copy} dari \en{host} ke \en{device}, pertama-tama \en{host} menyalin isi memori ke \en{staging buffer}. Kemudian, \en{device} membaca isi memori dari \en{staging buffer}. Di saat yang bersamaan, \en{host} menyalin isi memori lainnya ke bagian lain \en{staging buffer}, yang kemudian akan dibaca oleh \en{device}, dan begitu seterusnya \citep{wilt2013cuda}.

\section{Penggunaan Memori pada Pelatihan Model} \label{MemoryUsage}
...
Agar dapat meng-\en{improve} dengan baik, perlu dipertimbangkan \en{tensor} mana yang perlu di-\en{swap}. Apabila semua \en{tensor} di-\en{swap}, akan menyebabkan penurunan kinerja model karena \en{memory transfer overhead} yang besar. Menurut \en{paper} tersebut, \en{tensor}-\en{tensor} yang terbaik untuk di-\en{swap} adalah yang interval hingga diperlukannya kembali saat \en{backpropagation} paling lama. Harapannya, \en{overlapping} antara komputasi dengan \en{asynchronous memory copy} meningkat, sehingga mengurangi durasi pelatihan karena semakin banyak \en{memory copy} yang berjalan bersama dengan komputasi.

Eksperimen pada \en{paper} tersebut dilakukan dengan beberapa \en{pre-trained model} seperti ResNet, Inception, dan NMT, pada sebuah komputer dengan satu GPU (Tesla M40) dengan kapasitas memori 12GB. Menurut hasil eksperimen yang ditunjukkan Gambar~\ref{fig:SwapOutInResult} tersebut, didapatkan bahwa pada model-model yang diuji, ukuran \en{batch} dapat ditingkatkan hingga 3 kali, sedangkan penggunaan memori maksimum menurun hingga 2-3 kali lipat. Bahkan, model-model seperti ResNet-1001 dan ResNet-2000 yang sebelumnya tidak dapat dilatih karena \en{out of memory} (OOM), menjadi dapat dilatih. Di sisi lain, durasi \en{training} mengalami peningkatan kurang dari 10\%, karena adanya \en{memory transfer overhead} seperti yang telah disebutkan.

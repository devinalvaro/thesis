...
Menurut \citep{meng2017training}, \en{layer}-\en{layer} yang paling baik untuk di-\en{swap} adalah \en{layer}-\en{layer} awal, karena interval dari \en{forward pass} hingga digunakannya kembali saat \en{backpropagation} terpanjang. Dengan begitu, diharapkan bahwa \en{overlapping} antara komputasi dengan \en{memory swapping} meningkat, sehingga mengurangi waktu yang dibutuhkan (karena \en{data transfer} antara GPU dan CPU berjalan secara \en{asyncrhonous}).
...
Alasan lainnya adalah fitur \en{memory swapping} pada TensorFlow hanya terdapat pada \code{tf.nn.dynamic\_rnn}, sehingga dengan memilih model berarsitektur RNN, dapat dibandingkan secara langsung \en{memory swapping} hasil optimisasi terhadap \en{memory swapping} bawaan dari TensorFlow.

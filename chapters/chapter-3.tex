...
Salah satu \en{framework} pembelajaran mesin, TensorFlow, memiliki fitur \en{memory swapping}. Namun, terdapat potensi bahwa \en{memory swapping} di TensorFlow masih kurang optimal, seperti yang dijelaskan secara lebih detail di bagian selanjutnya.
...
Selanjutnya, akan dijelaskan mekanisme \en{memory swapping} di Tensorflow dari sisi yang lebih \en{low-level}.

\subsubsection{\en{Stack}}

Sedikit mengulang yang telah dijelaskan di Bagian~\ref{tensorflow}, program TensorFlow dapat dilihat sebagai sebuah \en{dataflow graph}, di mana \en{tensor} melewati sisi graf dan simpul graf adalah operasi (\en{op}) terhadap \en{tensor}-\en{tensor} yang melewatinya. Saat \en{forward pass} berlangsung, \en{output tensor} dari \en{op} tertentu diletakkan ke sebuah \en{stack} agar ketika \en{backpropagation}, \en{op} bersangkutan dapat mengambil kembali \en{tensor} yang diletakkan ke \en{stack} tadi dengan urutan yang benar. Perlu diperhatikan bahwa setiap \en{op} memiliki \en{stack}-nya sendiri.

\en{Stack} yang dimaksud adalah kelas \code{Stack} yang terdapat di \en{codebase} TensorFlow. Selain kelas tersebut, terdapat kelas \code{StackPushOp} dan \code{StackPopOp}, dua \en{op} yang bertanggung jawab meletakkan dan mengambil \code{tensor} seperti yang disebutkan pada paragraf sebelumnya. (Sebagai detail implementasi, sebenarnya \code{tensor} di sini adalah \en{handle} dari nilai \en{tensor} sesungguhnya yang terletak di memori GPU atau CPU.)

\subsubsection{\en{Swap Out}}
...
\begin{lstlisting}[language=C++, caption=\code{tensorflow/core/kernels/stack\_ops.cc}, label={lst:SwapOut}]
...
Terlihat pada Kode~\ref{lst:SwapOut} bahwa \code{StackPushOp} memiliki \en{method} \code{ComputeAsync} yang melakukan peletakkan \code{tensor} ke \code{stack}. \code{ComputeAsync} memiliki cara kerja sebagai berikut. Apabila \code{swap\_memory\_} bernilai \code{true} dan kondisi heuristiknya terpenuhi, maka dimulai pemindahan nilai \en{tensor} dari GPU ke memori CPU (\en{swap out}). Setelah pemindahan selesai, yang diletakkan ke atas \code{stack} adalah \en{instance} \code{tensor} baru (pada Kode~\ref{lst:SwapOut} dinamakan \code{cpu\_tensor}) sebagai \en{handle} terhadap nilai \en{tensor} yang sekarang terletak di memori CPU. Untuk memperjelas konteks, \code{tensor} ini nantinya digunakan dalam pemindahan kembali nilai \en{tensor} dari CPU ke GPU (\en{swap in}) ketika dibutuhkan saat \en{backpropagation}.
...
\begin{lstlisting}[language=C++, caption=\code{tensorflow/core/kernels/stack\_ops.cc}, label={lst:SwapIn}]
...
Dari Kode~\ref{lst:SwapIn} di atas, terlihat bahwa pada \code{StackPopOp}, TensorFlow memeriksa terlebih dahulu apakah \code{tensor} sebelumnya telah dipindahkan ke CPU. Jika iya, maka nilai \en{tensor} dipindahkan terlebih dahulu dari CPU ke GPU secara \en{asynchronous}. Jika tidak, maka nilai \en{tensor} dapat langsung dipakai karena sudah ada di GPU. Meskipun pemindahan dilakukan secara \en{asynchronous} (tidak mem-\en{block} CPU), namun penggunaan \code{stack} tetap ter-\en{block}, karena menunggu pemanggilan \code{done()} dilakukan. Dapat diperhatikan bahwa pemanggilan \code{done()} pada kasus \en{swapping in} dilakukan pada \en{callback}-nya (berarti setelah \en{swapping in} selesai).

Kembali menelaah kedua kode tersebut, dapat diambil beberapa kesimpulan. Terdapat heuristik yang harus dipenuhi untuk menentukan apakah \en{tensor} layak di-\en{swap out} ke CPU, yaitu ketika ukuran \en{tensor} melebihi \code{kCopyThreshold} (2048 \en{bytes}) dan persentase penggunaan memori GPU melebihi \code{kOccupancy} (70\%). Hal yang perlu diperhatikan juga adalah ketika heuristik terpenuhi, \en{tensor} yang di-\en{swap out} adalah \en{tensor} yang baru akan diletakkan ke atas \code{stack} (\en{tensor} terbaru). Selain itu, dari kode \code{StackPopOp} terlihat bahwa \en{swap in} baru dilakukan saat \en{tensor} akan dipakai.

\subsection{Potensi Optimisasi dengan \en{Memory Swapping}} \label{potential}
...
Bila melihat heuristik \en{memory swapping} di TensorFlow yang dijelaskan di bagian sebelumnya, terlihat bahwa yang dilakukan TensorFlow justru kebalikannya. Karena ketika heuristik terpenuhi, \en{swapping} dilakukan terhadap \en{tensor} saat ini (\en{tensor} terbaru), yang berarti interval hingga digunakannya kembali tidak terlalu lama. Di sinilah terletak potensi optimisasi \en{memory swapping} di TensorFlow tersebut.
...
Seperti yang disebutkan pada Bagian~\ref{potential}, ide optimisasi tersebut terinspirasi dari \citep{meng2017training}. Namun, sebenarnya \en{paper} tersebut menceritakan mekanisme dari \en{memory swapping} yang diimplementasikan sendiri oleh para penulis (bukan bawaan TensorFlow) beserta optimisasinya. Sedangkan fokus tugas akhir ini adalah mengoptimisasi \en{memory swapping} bawaan TensorFlow, sehingga pendekatan dan detail optimisasi yang dilakukan di tugas akhir ini berbeda dari \en{paper} tersebut, karena memang implementasi \en{memory swapping} bawaan TensorFlow berbeda jauh dari implementasi \en{memory swapping} di \en{paper} tersebut.

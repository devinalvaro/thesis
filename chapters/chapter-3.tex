...
Fokus penelitian di tugas akhir ini terletak pada \en{improvement} penggunaan memori untuk pelatihan model, khususnya di bagian \en{memory swapping} yang akan dijelaskan kemudian. Seperti yang dijelaskan di Bagian~\ref{MemoryUsage}, memori digunakan untuk me-\en{maintain} nilai-nilai seperti bobot, bias, hasil fungsi aktivasi (\en{output}), serta data yang sedang diproses. Kelebihan penggunaan memori dari kapasitas yang ada dapat mengakibatkan gagalnya pelatihan model karena masalah \en{out of memory} (OOM).
...
Melihat potensi \en{improvement} tersebut, fokus tugas akhir ini adalah meneliti dan mengembangkan \en{memory swapping} di TensorFlow sehingga lebih baik dari yang sudah ada, agar mengurangi durasi pelatihan dibandingkan dengan \en{memory swapping} bawaan TensorFlow.
...
\subsection{Potensi \en{Improvement} dengan \en{Memory Swapping}} \label{OptimizationPotential}
...
Dari penjelasan tersebut, terlihat potensi \en{improvement} penggunaan memori pada pelatihan model \en{deep learning}. Memori untuk \en{tensor}-\en{tensor} yang sedang tidak digunakan tersebut lebih baik bila didelegasikan ke \en{memory pool} yang lebih besar.
...
Setelah menganalisis permasalahan, didapatkan bahwa terdapat potensi \en{improvement} \en{memory swapping} di TensorFlow, dengan memprioritaskan \en{tensor} dengan interval waktu yang terlama hingga digunakannya kembali saat \en{backpropagation}. Selanjutnya, ide \en{improvement} tersebut perlu diimplementasikan ke TensorFlow.

Seperti yang disebutkan pada Bagian~\ref{OptimizationPotential}, ide \en{improvement} tersebut terinspirasi dari \citep{meng2017training}. Namun, sebenarnya \en{paper} tersebut menceritakan mekanisme dari \en{memory swapping} yang diimplementasikan sendiri oleh para penulis (bukan bawaan TensorFlow) beserta \en{improvement}-nya (dijelaskan secara lebih detail di Bagian~\ref{RelatedWorks}). Sedangkan fokus tugas akhir ini adalah meng-\en{improve} \en{memory swapping} bawaan TensorFlow, sehingga pendekatan dan detail \en{improvement} yang dilakukan di tugas akhir ini berbeda dari \en{paper} tersebut, karena memang implementasi \en{memory swapping} bawaan TensorFlow berbeda jauh dari implementasi \en{memory swapping} di \en{paper} tersebut.

Dengan demikian, diperlukan implementasi yang dapat meng-\en{improve} sesuai ide tersebut, namun terhadap mekanisme \en{memory swapping} bawaan TensorFlow yang dijelaskan di Bagian~\ref{SwappingMechanism}. Berikut akan dijelaskan rancangan implementasi yang dibuat oleh penulis.
...
Demikian \en{pseudocode} versi \en{improved} dalam melakukan \en{swapping out} terhadap \en{oldest tensor} ketimbang \en{tensor} saat ini. Selanjutnya adalah \en{pseudocode} \en{improvement} terhadap mekanisme \en{swapping in}.
...
Demikianlah rancangan implementasi oleh penulis untuk meng-\en{improve} \en{memory swapping} di TensorFlow. Pada \nameref{ImplementationAndEvaluation} dibahas mengenai implementasi dan hasil pengujian \en{improvement} tersebut.

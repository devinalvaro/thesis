...
void StackPopOp::ComputeAsync(OpKernelContext* ctx, DoneCallback done) {
  ...
  // ...
  static constexpr double kOccupancy = 0.9;
  if (stats.bytes_in_use > (stats.bytes_limit * kOccupancy)) {
    return;
  }
...
Terdapat satu lagi hal yang perlu dibahas, yaitu herustik pada \en{swapping in}. Seperti heuristik pada \en{swapping out}, \en{swapping in} tersebut dilakukan ketika memori GPU hampir penuh. Tetapi, \en{threshold} (\code{kOccupancy}) yang digunakan lebih tinggi, yaitu 0.9 ketimbang 0.7 pada \en{swapping out}.

Heuristik pada \en{swapping out} sama persis dengan versi bawaan TensorFlow. Namun, heuristik pada \en{swapping in} tidak ada pada versi bawaan TensorFlow (untuk lebih jelasnya dapat dibandingkan antara Kode~\ref{lst:SwapInPseudocode} dengan Kode~\ref{lst:SwapInPseudocodeOptimized}), sehingga tidak ada basis yang tentu untuk menentukan nilai \code{kOccupancy} pada \en{swapping in} tersebut. Dipilih nilai yang lebih tinggi dari 0.7, yaitu 0.9 untuk meningkatkan \en{overlapping} antara komputasi dengan \en{swapping in}. Pemilihan nilai yang lebih tinggi ini tidak menjadi masalah karena \en{swapping in} dilakukan saat \en{backpropagation}, di mana penggunaan memori terus menurun.

Demikian penjelasan mengenai implementasi optimisasi \en{memory swapping} di TensorFlow. Berikutnya dilakukan pengujian terhadap optimisasi ini.

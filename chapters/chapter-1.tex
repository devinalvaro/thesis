...
Pelatihan model \en{deep learning} membutuhkan berbagai sumber daya komputasi. Salah satunya adalah memori yang dibutuhkan untuk menyimpan berbagai nilai komputasi, seperti yang dijelaskan di Bagian~\ref{memoryusage}.

Kekurangan memori dapat mengakibatkan kegagalan pelatihan model, karena eror \en{out of memory} (OOM). Masalah tambahan mengenai persoalan ini adalah kecilnya memori GPU, relatif terhadap memori CPU. Seperti yang dijelaskan di Bagian~\ref{gpu}, GPU memang banyak digunakan untuk pelatihan model, namun ukuran memorinya cenderung lebih terbatas dari memori CPU, sehingga menambah \en{constraint} terhadap masalah OOM tersebut.
...
Akan tetapi, \en{memory swapping} cenderung meningkatkan durasi pelatihan karena adanya \en{memory copy overhead}, yaitu waktu yang dibutuhkan untuk memindahkan data antara CPU dan GPU. Maka, \en{memory swapping} yang optimal perlu selektif dalam memilih data mana yang di-\en{swap} agar pelatihan model dapat berjalan tanpa OOM, juga tanpa menambah durasi pelatihan secara terlalu besar \citep{meng2017training}
...
Melihat potensi optimisasi tersebut, fokus tugas akhir ini adalah meneliti dan mengoptimisasi \en{memory swapping} di TensorFlow sehingga lebih optimal dari yang sudah ada dilihat dari segi durasi pelatihan.

...
\en{Deep learning} adalah suatu teknik \en{machine learning} yang menggunakan \en{deep neural network} (DNN), yaitu \en{artificial neural network} (ANN) dengan jumlah \en{hidden layer} lebih dari satu. Berbagai arsitektur \en{deep learning} seperti \en{convolutional neural network} (CNN) dan \en{recurrent neural network} (RNN) telah banyak diaplikasikan pada berbagai bidang, seperti \en{computer vision} dan \en{speech recognition}.

\subsection{Arsitektur}

Arsitektur \en{deep learning} sangat beragam sehingga tidak dapat semuanya dibahas secara detil, oleh karena itu hanya akan dibahas secara singkat salah satu arsitektur \en{deep learning} yang paling relevan dengan tulisan ini, yaitu CNN.

Pada CNN, neuron-neuron pada suatu \en{layer} tidak selalu tersusun secara 1 dimensi, namun dapat tersusun secara n-dimensi. Selain itu, suatu neuron tidak memiliki hubungan satu-satu dengan neuron-neuron di \en{layer} selanjutnya. Melainkan, kumpulan \en{weight} dan \en{bias} beberapa neuron sekaligus menjadi input dari fungsi aktivasi ke sebuah neuron di \en{layer} selanjutnya. Hubungan antar layer pada CNN diilustrasikan oleh Gambar ~\ref{fig:FeatureMap}. Inilah yang disebut \en{feature map}, yaitu \en{mapping} dari kumpulan neuron di suatu layer ke suatu neuron di \en{layer} selanjutnya.
...
Padahal, peningkatan kecepatan \en{training} dengan GPU menjadi kecil apabila memori GPU tidak cukup, karena terjadi \en{memory bottleneck} \cite{dean2012large}. Solusi yang biasa digunakan adalah mengurangi ukuran \en{dataset} atau parameter model, meskipun hal ini dapat menurunkan performa.
